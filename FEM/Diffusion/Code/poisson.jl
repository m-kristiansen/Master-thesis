...
θ(x) = x[2] < 0 ? -acos(x[1]/r(x)) : acos(x[1]/r(x))
u(x) = sin((2*θ(x)+π)/3)*r(x)^(2/3)
...
function solver(mesh ;write::Bool=false)
    ...
    model = GmshDiscreteModel("/Users/martinkristiansen/Desktop/Master thesis/Meshes/nonconvex_poisson_"*string(mesh)*".msh")
...
function conv_test(figname)
    ...
    plot(h, [e, h, h.^(2/3)], xaxis=:log, yaxis=:log, label=["e_tot" "h" "h^(2/3)"],
...
conv_test("possion_nonconvex.pdf")
